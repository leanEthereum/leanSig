...
mod tests {
    ...
    #[test]
    fn test_public_key_to_bytes_ssz_equivalence() {
        let mut rng = rand::rng();
        let root = TestTH::rand_domain(&mut rng);
        let parameter = TestTH::rand_parameter(&mut rng);
        let public_key = GeneralizedXMSSPublicKey::<TestTH> { root, parameter };
        ...
        let canonical_bytes = public_key.to_bytes();
        let ssz_from_canonical = canonical_bytes.as_ssz_bytes();

        // Path 2: Direct SSZ encoding
        let ssz_direct = public_key.as_ssz_bytes();

        // Verify equivalence: ssz_encode(to_bytes()) == as_ssz_bytes()
        assert_eq!(
            ssz_from_canonical, ssz_direct,
            "PublicKey: ssz_encode(to_bytes()) must equal as_ssz_bytes()"
        );
    }

    #[test]
    fn test_signature_to_bytes_ssz_equivalence() {
        type PRF = ShakePRFtoF<7, 5>;
        type TH = PoseidonTweakW1L5;
        type MH = PoseidonMessageHashW1;
        const BASE: usize = MH::BASE;
        ...
        // Path 2: Direct SSZ encoding
        let ssz_direct = signature.as_ssz_bytes();

        // Verify equivalence: ssz_encode(to_bytes()) == as_ssz_bytes()
        assert_eq!(
            ssz_from_canonical, ssz_direct,
            "Signature: ssz_encode(to_bytes()) must equal as_ssz_bytes()"
        );
    }

    #[test]
    fn test_secret_key_to_bytes_ssz_equivalence() {
        type PRF = ShakePRFtoF<7, 5>;
        type TH = PoseidonTweakW1L5;
        type MH = PoseidonMessageHashW1;
        const BASE: usize = MH::BASE;
        ...
        // Path 1: Get canonical bytes via to_bytes(), then SSZ encode them
        let canonical_bytes = sk.to_bytes();
        let ssz_from_canonical = canonical_bytes.as_ssz_bytes();

        // Path 2: Direct SSZ encoding
        let ssz_direct = sk.as_ssz_bytes();

        // Verify equivalence: ssz_encode(to_bytes()) == as_ssz_bytes()
        assert_eq!(
            ssz_from_canonical, ssz_direct,
            "SecretKey: ssz_encode(to_bytes()) must equal as_ssz_bytes()"
        );
    }
}
